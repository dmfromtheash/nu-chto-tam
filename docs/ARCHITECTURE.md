...
При регистрации нового аккаунта текущая гостевая история открытий переносится в историю пользователя. Перенос основан на текущем `guest_id`: обновляются только строки `openings`, где `guest_id` совпадает и `user_id IS NULL`. Для таких строк `user_id` устанавливается на нового пользователя, а `guest_id` очищается, чтобы перенесённые открытия не оставались в гостевой истории после выхода.

Переносятся только `openings` как история открытий. `saved_cards`, `events`, `visitors`, `rate_limits` и аналитические агрегаты не переносятся.

Для будущего Fate Portrait / "Лор Личности" `openings` остаются источником истины для алгоритма, а сохранённые карточки и отображение коллекции остаются отдельным пользовательским слоем.
